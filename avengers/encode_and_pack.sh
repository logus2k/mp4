...
input_movie_file_name="${movie_name}.mp4"
manifest_file_name="${movie_name}.mpd"
...
# 360p
ffmpeg -i ${input_movie_file_name} -c:a copy \
  -vf "scale=-2:360" \
...
packager \
  ...
  in=${movie_name}.vtt,stream=text,output=${movie_name}_output.vtt \
  in=h264_baseline_360p_600.mp4,stream=video,output=h264_360p.mp4 \
  in=h264_main_480p_1000.mp4,stream=video,output=h264_480p.mp4 \
  in=h264_main_720p_3000.mp4,stream=video,output=h264_720p.mp4 \
  in=h264_high_1080p_6000.mp4,stream=video,output=h264_1080p.mp4 \
  --mpd_output ${manifest_file_name}.mpd
